<ul class="{{ $menu->getUlClass() }}">
    ...
    @foreach ($menu->getMenu() as $item)
        @if ($item->canShow())
            <li class="{{ $menu->getLiClass() }} {{ $item->addLiActiveClassIfActive() }} {{ $item->addLiOpenClassIfHaveActiveSubmenu() }}">
                <a href="{{ $item->getUrl() }}" id="{{ $item->id }}" class="{{ $item->getAClass() }} {{ $item->addAClassIfActive() }}" @if ($item->target) target="{{ $item->target }}" @endif>
